...
namespace lstg::Subsystem::Render::Effect::detail::LuaBridge
{
    ...
    {
    public:
        static bool IsValidIdentifier(std::string_view name) noexcept;
        static lstg::Subsystem::Render::detail::EffectScriptState* GetScriptState(Script::LuaStack& st);
    ...
    public:  // 枚举
...
#else
        using ScalarTypes = ShaderValueType::ScalarTypes;
        using FilterTypes = FilterTypes;
        using TextureAddressModes = TextureAddressModes;
        using SemanticNames = ShaderVertexLayoutDefinition::ElementSemanticNames;
        ...
        using BlendFactors = BlendFactors;
        using BlendOperations = BlendOperations;
        using ColorWriteMask = ColorWriteMask;
        using FillModes = FillModes;
        using CullModes = CullModes;
        using ComparisionFunctions = ComparisionFunctions;
#endif

    public:  // 方法
        LSTG_METHOD(constantBuffer)
        static ConstantBufferBuilder DefineConstantBuffer(Script::LuaStack& st, const char* name);

        LSTG_METHOD(importConstantBuffer)
        static ConstantBufferWrapper ImportGlobalConstantBuffer(Script::LuaStack& st, const char* name);

        LSTG_METHOD(texture1d)
        static TextureVariableBuilder DefineTexture1D(Script::LuaStack& st, const char* name);

        LSTG_METHOD(texture2d)
        static TextureVariableBuilder DefineTexture2D(Script::LuaStack& st, const char* name);

        LSTG_METHOD(texture3d)
        static TextureVariableBuilder DefineTexture3D(Script::LuaStack& st, const char* name);

        LSTG_METHOD(textureCube)
        static TextureVariableBuilder DefineTextureCube(Script::LuaStack& st, const char* name);

        LSTG_METHOD(vertexLayout)
        static VertexLayoutBuilder DefineVertexLayout(Script::LuaStack& st);

        LSTG_METHOD(vertexShader)
        static ShaderBuilder DefineVertexShader(Script::LuaStack& st, const char* source);

        LSTG_METHOD(pixelShader)
        static ShaderBuilder DefinePixelShader(Script::LuaStack& st, const char* source);

        LSTG_METHOD(pass)
        static EffectPassBuilder DefinePass(Script::LuaStack& st, const char* name);

        LSTG_METHOD(passGroup)
        static EffectPassGroupBuilder DefinePassGroup(Script::LuaStack& st, const char* name);

        LSTG_METHOD(effect)
        static EffectBuilder DefineEffect();
    };

    void RenderEffectLuaBridge(Script::LuaStack& stack);
}
